...
<form action="<?=BASE_URL."module/kategori/action.php?kategori_id=$kategori_id"; ?>" method="POST">
	...
	<div class="element-form">
		<label>Status</label>
		<span>
			<input type="radio" name="status" value="on" <?php if($status=="on"){echo "checked='true'";}?>>On</span>
			<input type="radio" name="status" value="off" <?php if($status=="off"){echo "checked='true'";}?> >Off</span>
	</div>
